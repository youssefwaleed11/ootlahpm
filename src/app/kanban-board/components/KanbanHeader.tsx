...
export default function KanbanHeader({ selectedProjectId, onProjectChange, projects, onTaskCreated }: KanbanHeaderProps) {
  const [filterPriority, setFilterPriority] = useState<string>('all');
  const [projectDropOpen, setProjectDropOpen] = useState(false);
  const [createModalOpen, setCreateModalOpen] = useState(false);
  const activeProject = projects.find(p => p.id === selectedProjectId) || projects[0];
  const canCreateTask = CURRENT_USER.role !== 'agent';
    ...
    <>
      {createModalOpen && (
        <CreateTaskModal
          ...
          onTaskCreated={(task) => { onTaskCreated?.(task); setCreateModalOpen(false); }}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="text-sm font-600 text-slate-800 max-w-[180px] truncate">{activeProject?.name}</span>
            ...
            {projectDropOpen && (
              <div className="absolute top-10 left-0 w-72 bg-white rounded-xl border border-slate-200 shadow-modal z-30 fade-in overflow-hidden">
                <div className="px-3 py-2 border-b border-slate-100">
                  <p className="text-xs font-600 text-slate-500 uppercase tracking-wider">Switch Project</p>
                </div>
                {projects.map(proj => (
                  <button
                    key={proj.id}
                    onClick={() => { onProjectChange(proj.id); setProjectDropOpen(false); }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <div className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
                          <div className="h-full bg-brand-orange rounded-full" style={{ width: `${proj.progress}%` }} />
                        </div>
                        <span className="text-[10px] text-slate-500 font-tabular">{proj.progress}%</span>
                      </div>
                    </div>
                    {proj.id === selectedProjectId && <Icon name="CheckIcon" size={14} className="text-brand-orange flex-shrink-0 mt-1" />}
                  </button>
                ))}
              </div>
            )}
            ...
            {['all', 'critical', 'high', 'medium', 'low'].map(p => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-500 transition-all duration-150 ${
                  filterPriority === p
                    ? p === 'all' ? 'bg-slate-800 text-white' : PRIORITY_COLORS[p] + ' ring-1 ring-offset-1 ring-current'
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="flex -space-x-2">
              ...
              <div className="w-7 h-7 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[10px] font-600 text-slate-600" style={{ zIndex: 6 }}>
                +4
              </div>
            </div>
            ...
            {canCreateTask && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-orange hover:bg-brand-orange-dark text-white text-xs font-600 rounded-lg transition-all duration-150 active:scale-95"
              >
