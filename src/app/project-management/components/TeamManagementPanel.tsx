...
export default function TeamManagementPanel({ onClose }: TeamManagementPanelProps) {
  ...
  const handleAddMember = (userId: string) => {
    ...
    setTeams((prev) =>
      prev.map((t) =>
        t.id === selectedTeam.id ? { ...t, memberIds: [...t.memberIds, userId] } : t
      )
    );
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-modal w-full max-w-2xl max-h-[85vh] flex flex-col slide-in-right">
        ...
        <div className="flex flex-1 overflow-hidden">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors duration-150 ${
                    selectedTeam.id === team.id
                      ? 'bg-orange-50 border-r-2 border-brand-orange'
                      : 'hover:bg-slate-50'
                  }`}
                >
                  <div
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className="flex-1 py-1 bg-brand-orange text-white text-xs font-600 rounded-lg hover:bg-brand-orange-dark transition-colors"
                    >
                      Create
                    </button>
                    <button
                      onClick={() => setCreatingTeam(false)}
                      className="flex-1 py-1 bg-slate-100 text-slate-600 text-xs font-600 rounded-lg hover:bg-slate-200 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {teamMembers.length} members · {selectedTeam.projectIds.length} projects
                </p>
              </div>
            </div>
            ...
            {leader && (
              <div>
                <p className="text-[10px] font-600 text-slate-400 uppercase tracking-wider mb-2">
                  Team Leader
                </p>
                <div className="flex items-center gap-3 px-3 py-2.5 bg-teal-50 rounded-xl border border-teal-100">
                  ...
                  <span className="text-[10px] font-700 px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">
                    Leader
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                {teamMembers.map((member) => (
                  <div
                    ...
                    className="flex items-center gap-2.5 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100 group"
                  >
                    ...
                    <span
                      className={`text-[10px] font-600 px-1.5 py-0.5 rounded-full flex-shrink-0 ${ROLE_BADGE[member.role]}`}
                    >
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {nonMembers.map((user) => (
                    <div
                      key={`add-${user.id}`}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl border border-dashed border-slate-200 hover:border-brand-orange/40 hover:bg-orange-50/30 transition-all duration-150 group"
                    >
